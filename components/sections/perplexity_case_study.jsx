...
const PERPLEXITY_TOC_SECTIONS = [
  { id: "per-01", number: "01", label: "Context" },
  { id: "per-02", number: "02", label: "Pricing" },
  { id: "per-03", number: "03", label: "Funnel" },
  { id: "per-04", number: "04", label: "Compete" },
  { id: "per-05", number: "05", label: "Crisis" },
  { id: "per-06", number: "06", label: "Finance" },
  { id: "per-07", number: "07", label: "Risks" },
  { id: "per-08", number: "08", label: "Verdict" },
];
...
const PplxMockup = ({ query, answer, sources, model = "Frontier model" }) => (
  <div style={{ background: "#fff", borderRadius: 14, border: "1.5px solid #e8e6e3", overflow: "hidden", boxShadow: "0 8px 32px rgba(0,0,0,.07)", width: "100%", maxWidth: 380 }}>
    <div style={{ padding: "10px 14px", borderBottom: "1px solid #f0eeeb", display: "flex", alignItems: "center", gap: 8 }}>
      ...
      <span style={{ font: "600 11px/1 'Outfit',sans-serif", color: "#1a1a1a" }}>Perplexity</span>
    ...
    <div style={{ padding: "12px 14px 8px", background: "#faf9f7" }}>
      ...
      <div style={{ font: "400 12px/1.4 'Outfit',sans-serif", color: "#1a1a1a" }}>{query}</div>
    </div>
    <div style={{ padding: "12px 14px" }}>
      ...
      <div style={{ font: "400 10.5px/1.6 'Outfit',sans-serif", color: "#555" }}>{answer}</div>
      ...
      <div style={{ position: "relative", width: "100%", paddingBottom: "70%", background: "#faf9f7", borderRadius: 8, overflow: "hidden" }}>
        ...
        {competitors.map((c, i) => (
          <div key={i} style={{ position: "absolute", left: `${c.x}%`, top: `${c.y}%`, transform: "translate(-50%, -50%)", textAlign: "center", zIndex: c.bold ? 3 : 1 }}>
            ...
            <div style={{ font: `${c.bold ? 600 : 500} ${c.bold ? 11 : 10}px/1.2 'Outfit',sans-serif`, color: c.color, whiteSpace: "nowrap" }}>{c.name}</div>
    ...
    <div style={{ background: "#fff", borderRadius: 10, padding: "24px 28px", border: "1px solid rgba(0,0,0,.05)", margin: "32px 0" }}>
      ...
      {gaps.map((g, i) => (
        <div key={i} style={{ display: "grid", gridTemplateColumns: "90px 1fr 32px", alignItems: "center", gap: 10, marginBottom: 10 }}>
          <span style={{ font: "500 10px/1 'Outfit',sans-serif", color: "#555", textAlign: "right" }}>{g.label}</span>
      ...
      <div style={{ display: "flex", gap: 16, marginTop: 14 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 5 }}><div style={{ width: 14, height: 6, background: "#d3d0ca", borderRadius: 2 }} /><span style={{ font: "400 9px/1 'Outfit',sans-serif", color: "#999" }}>Free</span></div>
        <div style={{ display: "flex", alignItems: "center", gap: 5 }}><div style={{ width: 14, height: 6, background: "#6C5CE7", borderRadius: 2, opacity: 0.85 }} /><span style={{ font: "400 9px/1 'Outfit',sans-serif", color: "#999" }}>Pro $20/mo</span></div>
      </div>
    </div>
  );
};
      ...
      {[
        ...
        <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 14, marginBottom: 12, position: "relative" }}>
          ...
          <div>
            ...
            <div style={{ font: "400 12px/1.4 'Outfit',sans-serif", color: "rgba(255,255,255,.7)" }}>{e.event}</div>
          </div>
        </div>
      ))}
    </div>
    ...
    {[
      ...
      <div key={i} style={{ marginBottom: 10 }}>
        <div style={{ width: `${s.pct}%`, minWidth: 200, background: s.color+"14", border: `1.5px solid ${s.color}`, borderRadius: 8, padding: "10px 14px", display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          <span style={{ font: "700 10px/1 'Outfit',sans-serif", color: s.color, letterSpacing: ".04em", flexShrink: 0 }}>{s.label}</span>
          <span style={{ font: "400 10px/1.3 'Outfit',sans-serif", color: "#666" }}>{s.desc}</span>
        </div>
      </div>
    ))}
  </div>
    ...
    <div style={{ background: "#fff", borderRadius: 10, padding: "20px 22px", border: "1.5px solid #2d8a4e" }}>
      ...
      {["Effectively unlimited Pro searches","Hundreds of Deep Research runs","Model selected = model received","Generous file uploads","No usage throttling"].map((t,i)=>(
        <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 6, padding: "3.5px 0" }}>
          ...
          <span style={{ font: "400 11px/1.5 'Outfit',sans-serif", color: "#555" }}>{t}</span>
        </div>
      ))}
    </div>
...
const App = () => {
  ...
  return (<>
    ...
    {/* CONTEXT */}
    <section id="per-01" className="sect">
      <div className="wrap">
    ...
    {/* PRICING */}
    <section id="per-02" className="sect" style={{ background:"var(--white)" }}>
      <div className="wrap">
        ...
        <div style={{ marginTop:36 }}>
          ...
          <div className="tier" style={{ border:"2px solid #6C5CE7" }}><div><div className="tier-tag" style={{ background:"#f0eeff",color:"#6C5CE7" }}>MOST POPULAR</div><div className="tier-price">$20<span style={{ font:"400 14px/1 'Outfit',sans-serif",color:"#999" }}>/mo</span></div></div><div><div className="tier-name">Pro</div><div className="tier-desc">Frontier OpenAI, Anthropic, Google, and xAI models with per-query switching. ~20 Deep Research/month, generous file uploads, image gen, discounted API access, ad-free.</div></div></div>
          <div className="tier"><div><div className="tier-tag" style={{ background:"#fffbea",color:"#B8860B" }}>POWER</div><div className="tier-price">$200<span style={{ font:"400 14px/1 'Outfit',sans-serif",color:"#999" }}>/mo</span></div></div><div><div className="tier-name">Max</div><div className="tier-desc">Frontier reasoning models from OpenAI, Anthropic, and Google. Much higher limits across all features, video generation (limited), significantly discounted API usage.</div></div></div>
          <div className="tier"><div><div className="tier-tag" style={{ background:"#eef3fc",color:"#54A0FF" }}>ENTERPRISE</div><div className="tier-price">$40+<span style={{ font:"400 14px/1 'Outfit',sans-serif",color:"#999" }}>/seat</span></div></div><div><div className="tier-name">Enterprise Pro / Max</div><div className="tier-desc">SSO, SCIM, SOC 2 Type II, role-based permissions, internal knowledge search. Higher tiers scale into the low hundreds per seat.</div></div></div>
        </div>
    ...
    {/* FUNNEL */}
    <section id="per-03" className="sect">
      <div className="wrap">
    ...
    {/* COMPETITIVE */}
    <section id="per-04" className="sect" style={{ background:"var(--white)" }}>
      <div className="wrap">
    ...
    {/* CRISIS */}
    <section id="per-05" className="sect">
      <div className="wrap">
        ...
        <h3 style={{ font:"400 24px/1.2 'Instrument Serif',serif",color:"#1a1a1a",marginBottom:14 }}>How Competitors Handle Limits</h3>
        <table className="tbl">
    ...
    {/* FINANCIALS */}
    <section id="per-06" className="sect" style={{ background:"var(--white)" }}>
      <div className="wrap">
    ...
    {/* RISKS */}
    <section id="per-07" className="sect">
      <div className="wrap">
    ...
    {/* VERDICT */}
    <section id="per-08" className="sect" style={{ background:"var(--white)" }}>
      <div className="wrap">
          ...
          {[
            ...
            <div key={i} style={{ background:"#faf9f7",borderRadius:10,padding:"22px 28px",marginBottom:12,border:"1px solid rgba(0,0,0,.05)" }}>
              <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:6,flexWrap:"wrap" }}>
                ...
                <span style={{ font:"600 14px/1.3 'Outfit',sans-serif",color:"#1a1a1a",flex:1,minWidth:120 }}>{r.title}</span>
